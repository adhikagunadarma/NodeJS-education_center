...
exports.create = (req, res) => {
  ...
  fs.writeFile(
    videosPathFolder + req.body.videoFileName,
    videoBase64data,
    "base64",
    (err) => {
      if (err) {
        res.status(500).send({
          statusMessage: "Error while saving video file : " + err.message,
  ...
  fs.writeFile(
    thumbnailsPathFolder + req.body.videoThumbnailName,
    thumbnailBase64data,
    "base64",
    (err) => {
      if (err) {
        res.status(500).send({
          statusMessage: "Error while saving thumbnail file : " + err.message,
          statusCode: -999,
        });
        console.log(err);
      } else {
  ...
  video
    ...
    .then((data) => {
      // res.send(data);
      res.status(200).send({
        statusMessage: "Sukses Membuat video",
        statusCode: 0,
      });
    })
    .catch((err) => {
...
exports.findAll = (req, res) => {
  ...
  Video.find(condition)
    ...
    .catch((err) => {
      res.status(500).send({
        statusMessage:
          err.message || "Some error occurred while retrieving the Video.",
        statusCode: -999,
      });
    });
};
...
exports.findAllByCourse = (req, res) => {
  ...
  Video.find(condition)
    .then(async (data) => {
      ...
      res.send({
        statusMessage:
          "Berhasil GET all videos by course name " +
          updatedList[0].videoCourse, // karena semua video berasal dr course yg sama
        statusCode: 0,
        data: updatedList,
      });
    })
        ...
        if (req.body.videoFile && req.body.videoFileName) {
          fs.unlink(videosPathFolder + data.videoFileName, (err) => {
            if (err) {
              ...
              const videoBase64data = req.body.videoFile.replace(
                /^data:.*,/,
                ""
              );
              fs.writeFile(
        ...
        if (req.body.videoThumbnail && req.body.videoThumbnailName) {
          fs.unlink(thumbnailsPathFolder + data.videoThumbnailName, (err) => {
            if (err) {
              ...
              const thumbnailBase64data = req.body.videoThumbnail.replace(
                /^data:.*,/,
                ""
              );
              ...
              fs.writeFile(
                ...
                (err) => {
                  if (err) {
                    res.status(500).send({
                      statusMessage:
                        "Error while saving new thumbnail file : " +
                        err.message,
                      statusCode: -999,
                    });
                    console.log(
                      "Error while saving new thumbnail file : " + err
                    );
                    return;
                  }
                }
...
exports.deleteAllByCourse = async (req, res) => {
  ...
  Video.find(condition)
    .then(async (data) => {
        ...
        Video.findByIdAndRemove(element.id)
          .then((data) => {
            if (!data) {
              ...
              // kalo sukses
              fs.unlink(
                thumbnailsPathFolder + data.videoThumbnailName,
                (err) => {
                  if (err) {
                    res.status(500).send({
                      statusMessage:
                        "Cannot Delete thumbnail video with id=" + id,
                      statusCode: -999,
                    });
                    console.log(err);
                    return;
      ...
      res.send({
        statusMessage:
          "Berhasil delete all videos with course name " + courseName,
        statusCode: 0,
      });
    })
    .catch((err) => {
